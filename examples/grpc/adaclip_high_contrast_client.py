"""AdaCLIP high-contrast example using gRPC.

This client demonstrates running AdaCLIP with high-contrast band selection via gRPC:
  - Resolves the adaclip_high_contrast trainrun config
  - Applies the config to build the pipeline (with high-contrast band selector)
...
def main(server_address: str = "localhost:50051") -> None:
    ...
    # Resolve the adaclip_high_contrast trainrun config
    resolved, config_dict = resolve_trainrun_config(
        stub,
        session_id,
        "adaclip_high_contrast",
        overrides=[],
    )
    print(
    ...
    print("Applied AdaCLIP high-contrast trainrun config")
