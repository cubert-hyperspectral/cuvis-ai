"""AdaCLIP supervised windowed false-RGB example using gRPC.

This client demonstrates running AdaCLIP with supervised windowed false-RGB band selection via gRPC:
  - Resolves the adaclip_supervised_windowed_false_rgb trainrun config
  - Applies the config to build the pipeline (with supervised windowed false-RGB band selector)
...
def main(server_address: str = "localhost:50051") -> None:
    ...
    # Resolve the adaclip_supervised_windowed_false_rgb trainrun config
    resolved, config_dict = resolve_trainrun_config(
        stub,
        session_id,
        "adaclip_supervised_windowed_false_rgb",
        overrides=[],
    )
    print(
    ...
    print("Applied AdaCLIP supervised windowed false-RGB trainrun config")
