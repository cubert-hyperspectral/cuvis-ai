"""Run SAM3 mask propagation via the gRPC Inference API.

Supports CU3S or RGB video sources and writes tracking results as video_coco JSON.
Prompt masks are decoded from COCO detection JSON segmentations and sent through
``InputBatch.mask`` on configured prompt/update frames only.
"""
...
DEFAULT_CU3S_PIPELINE = "configs/pipeline/sam3/sam3_mask_propagation.yaml"
DEFAULT_VIDEO_PIPELINE = "configs/pipeline/sam3/sam3_mask_propagation_video.yaml"
...
def _frame_prompt_mask(
    prompt_masks_by_frame: dict[int, np.ndarray],
    frame_id: int,
) -> torch.Tensor | None:
    label_map = prompt_masks_by_frame.get(int(frame_id))
    if label_map is None:
        return None
    return torch.from_numpy(np.array(label_map, copy=True)).unsqueeze(0).to(dtype=torch.int32)
...
def _build_inference_inputs(
    ...
    if prompt_mask is not None:
        kwargs["mask"] = helpers.tensor_to_proto(prompt_mask)
...
def run_client(
    *,
    ...
    prompt_masks_by_frame, _frame_hw_by_id, _default_hw = load_mask_prompt_schedule(
        detection_json,
        prompt_specs,
    )
    ...
    try:
        ...
        for batch in tqdm(loader, total=total_frames, desc="SAM3 mask gRPC", unit="frame"):
            ...
            prompt_mask = _frame_prompt_mask(prompt_masks_by_frame, frame_id)
...
@click.option(
    ...
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    required=True,
    help="COCO detection JSON containing segmentation for prompt selection.",
)
@click.option(
    "--prompt",
    "prompt_specs",
...
@click.option(
    ...
    default=Path("outputs/sam3_grpc_mask_propagation_results.json"),
    show_default=True,
