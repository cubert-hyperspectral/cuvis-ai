"""AdaCLIP CIR false-RG color example using gRPC.

This client demonstrates running AdaCLIP with CIR false-RG band selection via gRPC:
  - Resolves the adaclip_cir_false_rg_color trainrun config
  - Applies the config to build the pipeline (with CIR false-RG band selector)
...
def main(server_address: str = "localhost:50051") -> None:
    ...
    # Resolve the adaclip_cir_false_rg_color trainrun config
    resolved, config_dict = resolve_trainrun_config(
        stub,
        session_id,
        "adaclip_cir_false_rg_color",
        overrides=[],
    )
    print(
    ...
    print("Applied AdaCLIP CIR false-RG color trainrun config")
