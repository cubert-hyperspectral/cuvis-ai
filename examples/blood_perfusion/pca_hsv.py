"""HSV-colored full-spectrum PCA blood-perfusion video export for CU3S sessions."""
...
class SelectProjectedComponentNode(Node):
    ...
    def __init__(self, component_index: int = DEFAULT_COMPONENT_INDEX, **kwargs: object) -> None:
...
def _log_pca_component_selection(
    *,
    ...
    logger.info("Selected PCA component: component_index={}", resolved_component_index)
...
def export_pca_hsv_video(
    *,
    ...
    start_frame: int = 0,
    end_frame: int = -1,
    init_frames: int | None = DEFAULT_INIT_FRAMES,
    frame_rate: float | None = None,
    frame_rotation: int | None = None,
    save_pipeline_config: bool = False,
) -> Path:
    """Run CU3S -> PCA -> component selection -> scalar normalization -> HSV colormap -> MP4 export."""
    ...
    try:
        logger.info("Writing PCA HSV logs to {}", log_path)
        ...
        pipeline_name = (
            "PCA_PerFrame_HSV_Projection"
            if resolved_pca_mode == "per_frame"
            else "PCA_Global_HSV_Projection"
        )
        ...
        projection_normalizer = MinMaxNormalizer(
            eps=1.0e-6,
            use_running_stats=resolved_pca_mode == "global",
        ...
        logger.info(
            "Starting PCA HSV export: file={} mode={} processing_mode={} frames={} init_frames={} "
            "component_index={} fps={} colormap_min={} colormap_max={} device={}",
            cu3s_file_path,
            resolved_pca_mode,
            resolved_mode,
            target_frames,
        ...
        logger.success("PCA HSV export complete: {}", output_path)
...
@click.option(
    ...
    help=(
        ...
        "For Blood_Perfusion_Refl, component 1 is the recommended perfusion comparison view."
    ),
)
...
def main(
    ...
    """Export an HSV-colored PCA visualization video for the blood-perfusion CU3S session."""
    ...
    output_video_path = run_output_dir / "pca_hsv_projection.mp4"

    export_pca_hsv_video(
