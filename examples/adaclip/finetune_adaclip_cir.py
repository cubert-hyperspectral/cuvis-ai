...
def main(cfg: DictConfig) -> None:
    logger.info("=== Fine-tune AdaCLIP adapter layers (CIR) ===")
    ...
    output_dir = output_dir.parent / f"{output_dir.name}_cir"
    ...
    # ── Pipeline ──────────────────────────────────────────────────────
    pipeline = CuvisPipeline("finetune_adaclip_cir")
    ...
    tb = TensorBoardMonitorNode(
        ...
        run_name=f"finetune_adaclip_cir",
    )
    ...
    pipeline.save_to_file(
        ...
        metadata=PipelineMetadata(
            name=pipeline.name,
            description="Fine-tuned AdaCLIP adapter layers with CIR band selection on lentils.",
            tags=["finetune", "adaclip", "cir", "anomaly_detection"],
            author="cuvis.ai",
        ),
    )
