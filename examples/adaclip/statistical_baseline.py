...
Override Hydra params like data_root, train_ids, test_ids, or model_name, e.g.:
    ...
    uv run python examples/adaclip/statistical_baseline.py \\
...
DEFAULT_EXPERIMENT_NAME = "statistical_adaclip_baseline"
...
def main(cfg: DictConfig) -> None:
    ...
    logger.info("=== AdaCLIP baseline example ===")
    logger.info("Data root: {}", data_root)  # Changed from %s to {}
    logger.info("Splits: train={}, val={}, test={}", train_ids, val_ids, test_ids)  # Changed
    logger.info("Model: {} | Weights: {}", model_name, weight_name)  # Changed
    logger.info("Prompt: {}", prompt_text)  # Changed
    logger.info("Target anomaly class_id: {}", target_class_id)  # Changed from %d to {}
    ...
    logger.info(
        "Wavelength range: {:.1f}-{:.1f} nm", wavelengths.min(), wavelengths.max()
    )  # Changed

    logger.info("Available AdaCLIP weights: {}", list_available_weights())  # Changed
