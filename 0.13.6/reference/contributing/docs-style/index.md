...
The Nodes catalog at [`docs/catalogs/nodes/index.md`](https://cubert-hyperspectral.github.io/cuvis-ai/0.13.6/catalogs/nodes/index.md) is **generated at build time** by `scripts/generate_node_catalog.py` (registered as a `mkdocs-gen-files` script in `mkdocs.yml`). Each row in the rendered page is a collapsible `<details>` element keyed off the node class's metadata.
...
**For questions or suggestions**, see: [Contributing Guide](https://cubert-hyperspectral.github.io/cuvis-ai/0.13.6/reference/contributing/contributing/index.md)
