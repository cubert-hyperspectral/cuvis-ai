...
1. **Use `@package _global_`** for trainrun configs
1. **Use clear variable names** — `${pipeline.metadata.name}` not `${a}`
1. **Only override what you need** — don't duplicate entire configs
1. **Document complex compositions** with header comments
1. **Use `resolve=True`** when converting to dict: `OmegaConf.to_container(cfg, resolve=True)`
...
- [Hydra Basics](https://cubert-hyperspectral.github.io/cuvis-ai/0.10.0/reference/configuration/hydra-basics/index.md) — package directives, defaults list
- [Hydra Inheritance & Overrides](https://cubert-hyperspectral.github.io/cuvis-ai/0.10.0/reference/configuration/hydra-inheritance/index.md) — config inheritance, interpolation
