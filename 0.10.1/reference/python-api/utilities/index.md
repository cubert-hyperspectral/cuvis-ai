...
The embedded pipeline must declare a `plugins:` block (the standalone `configs/pipeline/` yamls already do); a trainrun whose resolved pipeline omits it is rejected at `LoadPipeline` with a `suggest-plugins-fix` hint.
...
The bundled `configs/trainrun/*.yaml` reference their pipeline by a path relative to the trainrun file. Resolve it against `trainrun_dir` (default the package `configs/trainrun`) and load the pipeline YAML directly. The bundled pipeline yamls are self-contained `PipelineConfig`s (nodes + `plugins:`), so a direct file load avoids the Hydra group-path wrapping a server-side `ResolveConfig(config_type="pipeline")` would add.
...
- [Client Connections & Sessions](https://cubert-hyperspectral.github.io/cuvis-ai/0.10.1/deployment/client-connections/index.md)
- [Client Workflows & Error Handling](https://cubert-hyperspectral.github.io/cuvis-ai/0.10.1/deployment/client-workflows/index.md)
- [Profiling & Performance](https://cubert-hyperspectral.github.io/cuvis-ai/0.10.1/workflows/profiling/index.md)
