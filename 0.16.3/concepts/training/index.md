...
The base class `unfreeze()` promotes buffers listed in `TRAINABLE_BUFFERS` to `nn.Parameter`; `freeze()` reverts them. See [Node System Deep Dive: TRAINABLE_BUFFERS](https://cubert-hyperspectral.github.io/cuvis-ai/0.16.3/concepts/node/#trainable_buffers) for the full mechanism.
