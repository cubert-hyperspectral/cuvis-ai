...
Every node available in cuvis-ai pipelines, in one place. Built-in nodes ship with the `cuvis_ai` package; plugin nodes and data modules come from separately-installable plugin manifests — see [Plugin Development](https://docs.cuvis.ai/latest/reference/plugin-development/overview/index.md).
...
[View plugin repo (v0.5.0)](https://github.com/cubert-hyperspectral/cuvis-ai-adaclip/tree/v0.5.0)
...
[View plugin repo (v0.3.0)](https://github.com/cubert-hyperspectral/cuvis-ai-unet/tree/v0.3.0)
...
[View plugin repo (v0.8.0)](https://github.com/cubert-hyperspectral/cuvis-ai-dinomaly/tree/v0.8.0)
...
`WeightedCrossEntropyLoss`cuvis_ai_inspecscrap.node.losseslossclassdifftorchtraincuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Class-weighted multi-class cross-entropy over flattened logits + integer targets.
...
| Port                     | Dtype     | Shape      | Description                                                                                                                               |
| ------------------------ | --------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `logits`                 | `float32` | `[-1, -1]` | Class logits [N, num_classes] (leading dims flattened to N)                                                                               |
| `targets`                | `int64`   | `[-1]`     | Integer class targets [N]; ignore_index entries are dropped                                                                               |
| `class_weights` optional | `float32` | `[-1]`     | Optional per-class weights [num_classes]; when connected they override the constructor weights (e.g. fed by InverseFrequencyClassWeights) |

Outputs

| Port   | Dtype     | Shape | Description             |
| ------ | --------- | ----- | ----------------------- |
| `loss` | `float32` | `any` | Scalar weighted CE loss |

[View plugin repo (v0.3.0)](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap/tree/v0.3.0)
...
```python
AnomalyDetectionMetrics(
    ap_thresholds=200, pixel_stride=1, **kwargs
)
```
...
`pixel_stride` subsamples the pixel grid on H and W before the metrics see it, so a full-frame validation step no longer allocates flattened copies of every pixel. It changes only how many pixels the metrics score, never the `scores` grid a decider, overlay or heatmap reads.
...
With `pixel_stride` above 1 the three pixel grids are subsampled to every s-th row and column first, so the reported values are computed on that subsample.
...
```python
AnomalyPixelStatisticsMetric(**kwargs)
```
...
```python
ComponentOrthogonalityMetric(**kwargs)
```
...
`DistinctLabelCount`cuvis_ai.node.metricsmetricevalinfermaskCount the distinct non-zero labels per frame in an integer label map.

#### DistinctLabelCount

```python
DistinctLabelCount(**kwargs)
```
...
Reports how many separate segments a label map contains, e.g. how many compartments survived a per-blob majority vote or how many clusters a frame holds. Emits the per-frame count both as a `count` tensor (for pipeline reads / notebook printing) and as `Metric` objects for training-time logging. Declares `ExecutionStage.ALWAYS` (and the `INFERENCE` tag) so it also runs under `Predictor` inference, not only validation / test.
...
```python
ExplainedVarianceMetric(**kwargs)
```
...
`MulticlassSegmentationMetrics`cuvis_ai_inspecscrap.node.metricsmetricclassevalcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Pixel accuracy + macro precision/recall/F1 + per-class recall from logits or predictions.
...
| Port                   | Dtype     | Shape      | Description                                                      |
| ---------------------- | --------- | ---------- | ---------------------------------------------------------------- |
| `logits` optional      | `float32` | `[-1, -1]` | Class logits [N, num_classes]; argmaxed internally (optional)    |
| `predictions` optional | `int64`   | `[-1]`     | Integer class predictions [N] (optional; provide this or logits) |
| `targets`              | `int64`   | `[-1]`     | Integer class targets [N]; ignore_index entries are dropped      |
...
| Port      | Dtype | Shape | Description            |
| --------- | ----- | ----- | ---------------------- |
| `metrics` | `any` | `any` | List of Metric objects |
...
```python
SelectorDiversityMetric(**kwargs)
```
...
```python
SelectorEntropyMetric(eps=1e-06, **kwargs)
```
...
[View plugin repo (v0.5.0)](https://github.com/cubert-hyperspectral/cuvis-ai-rtsam2/tree/v0.5.0)
...
[View plugin repo (v0.5.1)](https://github.com/cubert-hyperspectral/cuvis-ai-sam3/tree/v0.5.1)
...
`SpatialSpectralCNN2D`cuvis_ai_inspecscrap.node.classification.cnn2dmodelclasshsilearntorchcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Spatial-spectral 2D-CNN over a patch (bands as input channels).
...
| Port      | Dtype     | Shape              | Description                                                  |
| --------- | --------- | ------------------ | ------------------------------------------------------------ |
| `patches` | `float32` | `[-1, -1, -1, -1]` | Center-pixel patches [N, P, P, C] (P=1 for the spectral MLP) |

Outputs

| Port     | Dtype     | Shape      | Description                             |
| -------- | --------- | ---------- | --------------------------------------- |
| `logits` | `float32` | `[-1, -1]` | Per-patch class logits [N, num_classes] |
...
`SpectralMLPClassifier`cuvis_ai_inspecscrap.node.classification.mlpmodelclasshsilearntorchcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Three-layer per-pixel spectral MLP over the patch center pixel.
...
`SpectralSpatialCNN3D`cuvis_ai_inspecscrap.node.classification.cnn3dmodelclasshsilearntorchcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Spectral-spatial 3D-CNN over a patch, classifying the center pixel.
...
`ClassMapAccumulator`cuvis_ai_inspecscrap.node.class_map_accumulatorsinkclassmaskpostcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Scatter chunked patch predictions back into per-frame `[H, W]` class maps (sink).
...
| ---------- | --------- | ---------- | --------------------------------------- |
| `logits`   | `float32` | `[-1, -1]` | Per-patch class logits [N, num_classes] |
| `frame_id` | `int64`   | `[-1]`     | Source frame id per patch [N]           |
| `y`        | `int64`   | `[-1]`     | Pixel row per patch [N]                 |
| `x`        | `int64`   | `[-1]`     | Pixel column per patch [N]              |
| `height`   | `int64`   | `[-1]`     | Source frame height per patch [N]       |
| `width`    | `int64`   | `[-1]`     | Source frame width per patch [N]        |
...
`MontageColumnSink`cuvis_ai_inspecscrap.node.montage_sinksinkimgpostrgbcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Stitch captioned montage columns into one side-by-side image with a legend (sink).
...
| Port                 | Dtype     | Shape             | Description                                                                |
| -------------------- | --------- | ----------------- | -------------------------------------------------------------------------- |
| `frame`              | `float32` | `[-1, -1, -1, 3]` | Captioned column panels [B, H, W, 3] in [0, 1]                             |
| `column_index`       | `int64`   | `[-1]`            | Column position per panel [B]                                              |
| `label_rgb` optional | `any`     | `[-1, -1, -1, 3]` | Optional colourised label map per panel [B, H, W, 3]; drives legend counts |
...
`TensorBoardMonitorNode`cuvis_ai.node.monitorsinkevalmetatrainTensorBoard monitoring node for logging artifacts and metrics.
...
Runs during train, val and test (the class-level `EXECUTION_STAGES`); inference pipelines skip it, so loading a trained pipeline for prediction writes no TensorBoard directory. To log a trained pipeline's artifacts over a dataset, run it at the test stage (`Predictor.predict(stage=ExecutionStage.TEST)`). The log directory and the `SummaryWriter` are created on the first `forward` or `log` call, never at construction, so building the node touches no disk.
...
[View plugin repo (v0.8.0)](https://github.com/cubert-hyperspectral/cuvis-ai-dataloader/tree/v0.8.0)
...
`TiffDataNode`cuvis_ai_inspecscrap.node.datasourcehsicuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Adapt `tiff_paired` batch keys to the dtypes the channel selectors expect.
...
| Port          | Dtype | Shape              | Description                                                    |
| ------------- | ----- | ------------------ | -------------------------------------------------------------- |
| `cube`        | `any` | `[-1, -1, -1, -1]` | Hyperspectral cube [B, H, W, C] from the DataModule batch      |
| `wavelengths` | `any` | `[-1, -1]`         | Per-channel wavelengths [B, C] in nm from the DataModule batch |

Outputs

| Port          | Dtype     | Shape              | Description                                                    |
| ------------- | --------- | ------------------ | -------------------------------------------------------------- |
| `cube`        | `float32` | `[-1, -1, -1, -1]` | Hyperspectral cube [B, H, W, C] as float32                     |
| `wavelengths` | `int32`   | `[-1]`             | Wavelengths [C] in nm as numpy int32 (channel-selector parity) |
...
[View plugin repo (v0.4.3)](https://github.com/cubert-hyperspectral/cuvis-ai-augment/tree/v0.4.3)
...
```python
calibrate(scores, targets, *, num_candidates=256)
```

Refit `threshold` to F1-max on labelled validation scores.

`scores` is the decider-input tensor stacked over the split (`[N, H, W, C]`); `targets` the matching ground-truth mask. The sweep runs on `torch.sigmoid(scores)` in float32, exactly what `forward` compares against `threshold`, so the fitted value is reachable at runtime even when raw logits saturate the sigmoid. The optimum is moved to the midpoint of its F1 plateau (`_calibration.margin_below`) and written to the live attribute and to `hparams`; `pipeline.save_to_file` then carries it in the pipeline yaml. The `.pt` weights are unchanged, so load the saved yaml rather than the preset plus weights. Multi-channel input is reduced to the per-pixel max, which matches `forward` exactly for single-channel scores. Returns a report.

Raises: CalibrationError: shape mismatch, non-finite scores, or a single-class split.
...
`BlobMajorityVote`cuvis_ai_inspecscrap.node.deciderstransformclassmaskpostcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Relabel each object (instance / connected component) to its majority predicted class.
...
| Port                  | Dtype   | Shape          | Description                                                               |
| --------------------- | ------- | -------------- | ------------------------------------------------------------------------- |
| `predictions`         | `int64` | `[-1, -1, -1]` | Per-pixel class predictions [B,H,W]                                       |
| `instances` optional  | `int64` | `[-1, -1, -1]` | Object id per pixel [B,H,W]; ignore_instance is left unchanged (optional) |
| `foreground` optional | `int64` | `[-1, -1, -1]` | Foreground mask [B,H,W] (>0) to derive instances if none given (optional) |

Outputs

| Port          | Dtype   | Shape          | Description                      |
| ------------- | ------- | -------------- | -------------------------------- |
| `predictions` | `int64` | `[-1, -1, -1]` | Object-voted predictions [B,H,W] |
...
| `freeze_running_bounds_after_frames` | \`int  | None\`                                                                                                                                                                                                                                                                                                                                                                                     | When norm_mode='running', stop updating running_min/running_max after this many frames. None keeps legacy behavior (never freeze). Default 20. |
| `running_warmup_frames`              | `int`  | Number of initial running frames to normalize per-frame while collecting bounds. Set to 0 for fully stable live rendering from the first frame. Default 10. The frame counter is saved with the node's weights, so a checkpoint restored for inference does not warm up again or move its bounds; checkpoints written before the counter was saved are treated as warm and frozen on load. | `_WARMUP_FRAMES`                                                                                                                               |
...
`DeepSVDDCenterTracker`cuvis_ai.node.anomaly.deep_svddtransformanominferstatefultorchtrainTrack and expose Deep SVDD center statistics with optional logging.
...
`InverseFrequencyClassWeights`cuvis_ai_inspecscrap.node.lossestransformclasstorchtraincuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Entry node that fits inverse-frequency class weights from the training-label distribution.
...
| Port               | Dtype   | Shape  | Description                                                     |
| ------------------ | ------- | ------ | --------------------------------------------------------------- |
| `targets` optional | `int64` | `[-1]` | Integer class targets [N] from the batch (ignore_index dropped) |

Outputs

| Port            | Dtype     | Shape  | Description                                                      |
| --------------- | --------- | ------ | ---------------------------------------------------------------- |
| `class_weights` | `float32` | `[-1]` | Inverse-frequency class weights [num_classes], fitted in Phase 1 |
...
`LegendStripNode`cuvis_ai_inspecscrap.node.legendtransformvidcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Append a horizontal class-colour legend strip below the input frame.
...
| Port                 | Dtype     | Shape             | Description                                                                                                                      |
| -------------------- | --------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `frame`              | `float32` | `[-1, -1, -1, 3]` | Input frame [B, H, W, 3] in [0, 1]                                                                                               |
| `label_rgb` optional | `any`     | `[-1, -1, -1, 3]` | Optional per-frame colorized label map [B, H, W, 3]; uint8 [0, 255] or float32 [0, 1], used to compute per-class instance counts |

Outputs

| Port    | Dtype     | Shape             | Description                                                      |
| ------- | --------- | ----------------- | ---------------------------------------------------------------- |
| `frame` | `float32` | `[-1, -1, -1, 3]` | Frame with legend strip appended [B, H+legend_h, W, 3] in [0, 1] |
...
`PatchSampler`cuvis_ai_inspecscrap.node.patch_samplertransformhsiprestochcuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Extract labeled center-pixel patches from a cube + integer target map.
...
| Port      | Dtype     | Shape              | Description                     |
| --------- | --------- | ------------------ | ------------------------------- |
| `cube`    | `float32` | `[-1, -1, -1, -1]` | Hyperspectral cube [B,H,W,C]    |
| `targets` | `int64`   | `[-1, -1, -1]`     | Per-pixel class targets [B,H,W] |

Outputs

| Port      | Dtype     | Shape              | Description                    |
| --------- | --------- | ------------------ | ------------------------------ |
| `patches` | `float32` | `[-1, -1, -1, -1]` | Center-pixel patches [N,P,P,C] |
| `labels`  | `int64`   | `[-1]`             | Center-pixel class labels [N]  |

[View plugin repo (v0.3.0)](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap/tree/v0.3.0)

`PerChannelStandardizer`cuvis_ai_inspecscrap.node.normalizationtransformhsinormprecuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Standardize patches per spectral band with global training stats fitted in Phase 1.
...
| Port         | Dtype     | Shape              | Description                                |
| ------------ | --------- | ------------------ | ------------------------------------------ |
| `normalized` | `float32` | `[-1, -1, -1, -1]` | Per-band standardized patches [N, P, P, C] |
...
| `freeze_running_bounds_after_frames` | \`int   | None\`                                                                                | Stop updating running bounds after this many frames. Default 20; None keeps unbounded accumulation. |
...
The node recomputes its cutoff from each frame's own scores, so there is no absolute threshold to fit - the sweep finds the F1-best flagged-pixel fraction instead (per frame, over the full `[H, W, C]` tensor, matching the runtime `torch.quantile` with the default `reduce_dims`). The result is written to the live attribute and to `hparams`; `pipeline.save_to_file` then carries it in the pipeline yaml (the `.pt` weights are unchanged). Decisions are scored on the per-pixel max over channels, which matches `forward` exactly for single-channel scores.

Raises: CalibrationError: shape mismatch, non-finite scores, a single-class split, or a `reduce_dims` that keeps a real (size > 1) non-batch axis out of the quantile, a decision rule the per-frame sweep cannot reproduce.
...
`RgbLabelToClassIndex`cuvis_ai_inspecscrap.node.labelstransformclassmaskprecuvis_ai_inspecscrap[↗](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap "Open plugin repo")Map a colorized RGB label map to integer paper-class targets (fail-closed).
...
| Port        | Dtype | Shape             | Description                                                      |
| ----------- | ----- | ----------------- | ---------------------------------------------------------------- |
| `label_rgb` | `any` | `[-1, -1, -1, 3]` | Colorized label map [B, H, W, 3], uint8 [0,255] or float32 [0,1] |

Outputs

| Port      | Dtype   | Shape          | Description                                                        |
| --------- | ------- | -------------- | ------------------------------------------------------------------ |
| `targets` | `int64` | `[-1, -1, -1]` | Per-pixel class index [B, H, W]; background/unknown = ignore_index |
...
`TwoStageBinaryDecider`cuvis_ai.node.deciders.two_stage_decidertransformclassnumpypostTwo-stage binary decider: optional image-level gate + pixel mask.
...
Stage 1 is the image gate: the mean of the top `top_k_fraction` per-pixel scores must reach `image_threshold` or the frame gets a blank mask. `image_threshold=None` (the default) turns the gate off, so every frame reaches stage 2; a training preset ships it as `null` and :meth:`calibrate` (the in-training calibration phase, or the `calibrate-thresholds` CLI) fills in the value fitted on the labelled validation split.

Stage 2 uses the calibrated absolute `pixel_threshold` (raw score space) when one is set and otherwise falls back to the per-frame `quantile` cutoff, which flags a fixed fraction of every gated frame. With both `image_threshold` and `pixel_threshold` unset this node is `QuantileBinaryDecider` for `[B, H, W, 1]` input, decision for decision.
...
Stage 1 (only when `image_threshold` is set): the image score is the mean of the top-k per-pixel scores (`k = ceil(top_k_fraction * pixels)`, at least 1). A frame below the threshold gets a blank mask. With `image_threshold=None` every frame goes straight to stage 2 and no top-k statistics are computed.

Stage 2: the calibrated absolute `pixel_threshold` when set, otherwise the per-frame `quantile` cutoff over the frame's own scores.
...
Multi-channel inputs reduce to the per-pixel max across channels before either comparison. Debug logging is lazy: nothing is formatted, and no tensor is synchronized for logging, unless the debug level is enabled.
...
Runs the joint 2-D sweep (image gate x absolute pixel cutoff, raw score space, with gated-out frames contributing their full ground truth as misses). Sets the F1-max image gate and, at that gate, the F1-max absolute pixel threshold - moving stage 2 off the fixed-fraction quantile onto a cutoff that tracks anomaly size. Both values are the midpoints of their F1 plateaus (`_calibration.margin_below`); the pixel cutoff is exact in float32, the dtype `forward` compares in, and the gate uses the very `frame_image_score` statistic `forward` computes. The values go to the live attributes and to `hparams`; `pipeline.save_to_file` then carries them in the pipeline yaml. The `.pt` weights are unchanged, so load the saved yaml rather than the preset plus weights. `scores` is the decider-input tensor stacked over the split (`[N, H, W, C]`); `targets` the ground-truth mask. The report also carries the on-point optima and the joint optimum, which may beat the conditional one.
...
`AnomalyMask`cuvis_ai.node.anomaly_visualizationvisualizeranomevalmaskVisualize anomaly detection with GT and predicted masks.
...
Runs during validation and test, where the TensorBoard sink consumes the artifacts; inference pipelines skip it (`Predictor.predict(stage=ExecutionStage.TEST)` runs it over a labeled split).
...
`ChannelSelectorFalseRGBViz`cuvis_ai.node.anomaly_visualizationvisualizerevalhsirgbtrainVisualize false RGB output from channel selectors with optional mask overlay.
...
`ChannelWeightsViz`cuvis_ai.node.anomaly_visualizationvisualizerevalhsirgbtrainVisualize channel mixer weights as a heatmap.
...
`CubeRGBVisualizer`cuvis_ai.node.pipeline_visualizationvisualizerevalhsirgbCreates false-color RGB images from hyperspectral cube using channel weights.

#### CubeRGBVisualizer

```python
CubeRGBVisualizer(up_to=5, **kwargs)
```
...
Runs during validation and test (its artifacts go to the TensorBoard sink); inference pipelines skip it.
...
`ImageArtifactVizBase`cuvis_ai.node.anomaly_visualizationvisualizeranomevalimgtrainBase class for visualization nodes that produce image artifacts.

#### ImageArtifactVizBase

```python
ImageArtifactVizBase(
    max_samples=4, log_every_n_batches=1, **kwargs
)
```
...
`PCAVisualization`cuvis_ai.node.pipeline_visualizationvisualizerevalhsirgbVisualize PCA-projected data with scatter and image plots.
...
`PipelineComparisonVisualizer`cuvis_ai.node.pipeline_visualizationvisualizerevalhsirgbtrainTensorBoard visualization node for comparing pipeline stages.
...
`RGBAnomalyMask`cuvis_ai.node.anomaly_visualizationvisualizeranomevalmaskrgbVisualize anomaly detection with GT and predicted masks on RGB images.
...
`ScoreHeatmapVisualizer`cuvis_ai.node.anomaly_visualizationvisualizeranomevalrgbLog LAD/RX score heatmaps as TensorBoard artifacts.
...
Runs during validation and test (the TensorBoard sink is its consumer); inference pipelines skip it.
