...
If you have not installed cuvis-ai yet, follow the [Installation Guide](https://docs.cuvis.ai/latest/get-started/installation/index.md) first.
...
This drops `.cu3s` files under `data/XMR_Demo_Blood_Perfusion/`. Other demo datasets live in the [datasets catalog](https://docs.cuvis.ai/latest/catalogs/datasets/index.md).
...
Reading `.cu3s` files goes through the `cuvis-ai-dataloader` plugin, which `uv sync` does not install (and removes again on a later sync). Provision it once for this pipeline; its `cu3s` extra also needs the system-wide C++ Cuvis SDK from the [Installation Guide](https://docs.cuvis.ai/latest/get-started/installation/index.md):
...
Then point the pipeline at any `.cu3s` file from the dataset — `Auto_005.cu3s` is a good first run. The pipeline lives inside the packaged `configs/` tree, so `restore-pipeline` finds the sibling `configs/plugins/` catalog with the `cu3s` data module's manifest on its own (`--plugins-dir` is for pipelines stored elsewhere):
...
Your browser does not support embedded video. The clip lives at [img/ndvi-projection.mp4](https://docs.cuvis.ai/latest/get-started/img/ndvi-projection.mp4). \](../img/ndvi-projection.mp4)

## 4. Where to go next

- **[Blood Perfusion Tutorial](https://docs.cuvis.ai/latest/tutorials/statistical/blood-perfusion/index.md)** — the spectral theory, plus an extension that adds a qualitative SpO₂ proxy.
...
- **[Concepts](https://docs.cuvis.ai/latest/concepts/index.md)** — the mental model: nodes, ports, pipelines, training.
- **[Workflows → Build Pipeline (YAML)](https://docs.cuvis.ai/latest/workflows/build-pipeline-yaml/index.md)** — author a YAML pipeline from scratch.
- **[Workflows → Restore Pipeline](https://docs.cuvis.ai/latest/workflows/restore-pipeline/index.md)** — full reference for the `restore-pipeline` CLI.
- **[Tutorials](https://docs.cuvis.ai/latest/tutorials/index.md)** — the rest of the gallery: RX Anomaly, Channel Selection, Deep SVDD, AdaCLIP.
