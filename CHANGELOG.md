...
- **The public docs are runnable for anonymous readers again.** Every link into the private `cuvis-ai-cookbook` repo is replaced by the packaged pipeline/trainrun YAML or the notebook it stood for (the gRPC client-example links, which had no public equivalent, are dropped); the quickstart and the first-pipeline page provision the `cu3s` data module with `uv run provision`, the quickstart downloads with `dataset download lentils`, fits the RX detector with the `StatisticalTrainer` recipe and points every command at `Lentils_000.cu3s`, the file the dataset ships (the same fix lands in the statistical-training recipe and the gRPC client example); the 12 trainruns the docs now point at write `outputs/<name>` instead of a literal `outputs\<name>` directory. `tests/docs/test_no_private_repo_links.py` guards both the cookbook links and that every `github.com/.../cuvis-ai` blob or tree link resolves to a path in the repo.
