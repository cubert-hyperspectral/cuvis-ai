...
- torchcodec forks with torch: `>=0.11.1,<0.12` beside the cu128 torch 2.11 (x86_64 Linux, Windows) and `>=0.16.0` beside the cu130 torch 2.14 (aarch64 Linux). Every 0.12+ torchcodec wheel on Linux links the CUDA 13 runtime (`libcudart.so.13`, `libnvrtc.so.13`, `libnvjpeg.so.13`), so one 0.16 for both forks fails at import next to a cu128 torch; the import test in this release is what caught it. Children inherit the forked floors.
- `tests/test_pyproject_torch_sources.py` guards the fork: the markers parse and partition Jetson / Linux x86_64 / Windows, the base requirements mirror them, the lock's resolution markers select exactly one torch and torchvision entry per platform and Python minor, the cu130 entries ship an aarch64 wheel and the cu128 entries an x86_64 and a Windows wheel for every supported CPython, torchcodec has one entry per environment (0.11.x beside cu128, 0.12+ beside cu130), and `import torchcodec` plus its `VideoDecoder` load against the installed torch. `tests/plugins/test_builtin_manifest_pin.py` ties the `cuvis_ai_builtin` manifest tag to the newest CHANGELOG version; the pin had stayed at v0.16.0 through six releases.
