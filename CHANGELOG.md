...
- Renamed the helper-scripts package `tools/` → `scripts/`. Updated `[project.scripts]`, `tool.setuptools.packages.find`, MkDocs macros, codecov ignore, `.gitignore`, git hooks, and copilot-instructions.
- Bundled ffmpeg via `imageio-ffmpeg`. `ToVideoNode` resolves the binary from the wheel by default — no system install needed. Override with `CUVIS_AI_FFMPEG_BIN` (e.g. for `h264_nvenc` / `vaapi` / `amf`). Blood-perfusion notebook MP4 export gains `+faststart`.
- Removed `configs/plugins/registry.yaml`. Use per-plugin manifests (`configs/plugins/<plugin>.yaml`) — bumps no longer funnel through one file. Combined-manifest pattern in [external nodes](docs/catalogs/nodes/external.md#combined-manifest).
- **Docs IA restructure** (ALL-5655). Nine top-level sections — Home, Get Started, Concepts, Tutorials, Catalogs, Workflows, Agentic Integration, Deployment, Reference — ordered as a learning path. Every page is one of four types: Tutorial, Workflow, Catalog, or Reference. New content: agentic-integration section, datasets catalog mirrored from HuggingFace (`scripts/sync_hf_readmes.py`), notebook tutorial gallery linking to `cuvis-ai-cookbook`, `get-started/first-pipeline.md`, `workflows/{statistical,gradient}-training.md`. Major moves: `user-guide/{installation,quickstart}` → `get-started/`; `how-to/*` → `workflows/`; `node-catalog/*` → `catalogs/nodes/*`; `config/*` → `reference/configuration/`; `api/*` → `reference/python-api/`; `plugin-system/*` → `reference/plugin-development/`; `development/*` → `reference/contributing/`; `grpc/*` + `use_cases/grpc-workflow.md` → `deployment/`. Removed `docs/use_cases/` (long narrative pages replaced by short tutorial blurbs), `user-guide/configuration.md` stub, duplicate `plugin-system/overview.md`. Nav features swapped from `tabs`/`sections` to `indexes`. `llmstxt.sections` rewritten to the new tree. ~117 internal links rewired (`scripts/rewire_internal_links.py`). `mkdocs build --strict` passes 0 warnings. **All URLs change** — no redirects in this pass; external inbound links to old paths will 404.
