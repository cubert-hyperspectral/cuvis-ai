...
- **Execution stages are declared on the node class** (needs cuvis-ai-core 0.14.1, `EXECUTION_STAGES`). Losses, metrics, the TensorBoard sink and the artifact visualizers declare their stages on the class body; the copied `consume_base_kwargs` blocks and the `LossNode` assert are gone. A pipeline yaml can still move a node per instance with `hparams: {execution_stages: [inference]}` (string values are coerced, unknown names raise at load); the override lives in the yaml text only, `serialize()` does not write it back.
...
- **CLI inference writes no TensorBoard artifacts for these visualizers any more** (`Predictor`, `restore-pipeline`). Opt back in per pipeline with the yaml `execution_stages` hparam on the monitor and the visualizer feeding it, or call `Predictor.predict(stage=ExecutionStage.TEST)`.
...
- Bumped the `cuvis_ai_builtin` manifest pin v0.13.8 -> v0.13.9 so composed child environments install this release.
- Requires `cuvis-ai-core>=0.14.1`.
