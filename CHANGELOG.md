...
- Bumped pinned plugin tags to latest published patches: adaclip `v0.1.2 → v0.1.3`, ultralytics `v0.1.0 → v0.1.1`, deepeiou `v0.1.0 → v0.1.1`, trackeval `v0.1.0 → v0.1.1`, sam3 `v0.1.3 → v0.1.4`. Picks up `_category` / `_tags` palette annotations and the `cuvis-ai-schemas>=0.4.0` floor across all upstream plugins.
...
- Moved `notebooks/blood_perfusion/nd_blood_perfusion.ipynb` to `notebooks/use_cases/blood_perfusion.ipynb` and added `notebooks/use_cases/uc_object_tracking.ipynb`.
- Expanded `uc_object_tracking.ipynb` with four progressive phases sharing the prepare → build → run → watch rhythm: §9 CIR false-RGB rendering, §10 SAM3 mask propagation on RGB video, §11 SAM3 mask propagation on CIR video, §12 active tracking with invisible spectral ink. The §5–7 text-prop pipeline now also emits a clean `rgb_video.mp4` and a COCO `tracking_results.json` so §10/§11 can read frames straight from the cached MP4.
