...
Every node available in cuvis-ai pipelines, in one place. Built-in nodes ship with the `cuvis_ai` package; plugin nodes and data modules come from separately-installable plugin manifests — see [Plugin Development](https://cubert-hyperspectral.github.io/cuvis-ai/0.11.0/reference/plugin-development/overview/index.md).
...
anomaugbatchedbboxclassdetdiffdim-redembevalhsiimginferkplearnmaskmetanormnumpypostprereconrgbsegstatefulstochstreamtexttorchtracktrainvid
...
[View plugin repo (v0.2.0)](https://github.com/cubert-hyperspectral/cuvis-ai-dinomaly/tree/v0.2.0)
...
[View plugin repo (v0.2.1)](https://github.com/cubert-hyperspectral/cuvis-ai-inspecscrap/tree/v0.2.1)
...
[View plugin repo (v0.1.3)](https://github.com/cubert-hyperspectral/cuvis-ai-trackeval/tree/v0.1.3)
...
[View plugin repo (v0.2.0)](https://github.com/cubert-hyperspectral/cuvis-ai-deepeiou/tree/v0.2.0)
...
[View plugin repo (v0.1.7)](https://github.com/cubert-hyperspectral/cuvis-ai-sam3/tree/v0.1.7)
...
| Port                | Dtype     | Shape            | Description                                                      |
| ------------------- | --------- | ---------------- | ---------------------------------------------------------------- |
| `rgb_frame`         | `float32` | `[1, -1, -1, 3]` | RGB frame [1,H,W,3] in float32 with values in [0,1].             |
...
`SAM3TrackerInference`cuvis_ai_sam3.nodemodelbatchedinferlearnmaskrgbsegstatefultorchtrackvidsam3[↗](https://github.com/cubert-hyperspectral/cuvis-ai-sam3 "Open plugin repo")Abstract base for streaming SAM3 propagation nodes.
...
[View plugin repo (v0.1.3)](https://github.com/cubert-hyperspectral/cuvis-ai-ultralytics/tree/v0.1.3)
...
[View plugin repo (v0.2.0)](https://github.com/cubert-hyperspectral/cuvis-ai-dataloader/tree/v0.2.0)
...
`MultiCu3sDataModule`cuvis_ai_dataloader.data.datamodule_cu3s_multisourcedata modulecuvis_ai_dataloader[↗](https://github.com/cubert-hyperspectral/cuvis-ai-dataloader "Open plugin repo")Multiple cu3s recordings driven by an external splits.csv.

Multiple cu3s recordings driven by an external splits.csv.
...
[View plugin repo (v0.3.1)](https://github.com/cubert-hyperspectral/cuvis-ai-augment/tree/v0.3.1)
...
`AdaCLIPDetector`cuvis_ai_adaclip.node.adaclip_nodeunspecifiedadaclip[↗](https://github.com/cubert-hyperspectral/cuvis-ai-adaclip "Open plugin repo")AdaCLIP zero-shot anomaly detector node (plugin version).
...
[View plugin repo (v0.1.5)](https://github.com/cubert-hyperspectral/cuvis-ai-adaclip/tree/v0.1.5)

`ByteTrack`cuvis_ai_bytetrack.nodeunspecifiedbytetrackWrapper around yolox.tracker.BYTETracker for cuvis.ai pipelines.

Wrapper around yolox.tracker.BYTETracker for cuvis.ai pipelines.

Inputs

| Port                           | Dtype     | Shape          | Description                                                   |
| ------------------------------ | --------- | -------------- | ------------------------------------------------------------- |
| `bboxes`                       | `float32` | `[1, -1, 4]`   | Detection bounding boxes [1, N, 4] xyxy in image coordinates. |
| `category_ids`                 | `int64`   | `[1, -1]`      | Detection category IDs [1, N].                                |
| `confidences`                  | `float32` | `[1, -1]`      | Detection confidence scores [1, N].                           |
| `spectral_signatures` optional | `float32` | `[-1, -1, -1]` | Per-detection spectral signatures [B, N, C]. Optional.        |
| `spectral_valid` optional      | `int32`   | `[-1, -1]`     | Validity mask [B, N]. Optional.                               |
| `spectral_std` optional        | `float32` | `[-1, -1, -1]` | Per-detection spectral std [B, N, C]. Optional.               |

Outputs

| Port           | Dtype     | Shape        | Description                                                    |
| -------------- | --------- | ------------ | -------------------------------------------------------------- |
| `bboxes`       | `float32` | `[1, -1, 4]` | Pass-through bounding boxes [1, N, 4] xyxy.                    |
| `track_ids`    | `int64`   | `[1, -1]`    | Track IDs aligned to input detections [1, N], -1 if unmatched. |
| `confidences`  | `float32` | `[1, -1]`    | Pass-through detection confidences [1, N].                     |
| `category_ids` | `int64`   | `[1, -1]`    | Pass-through category IDs [1, N].                              |

`CategoryFilterNode`cuvis_ai_detr.nodeunspecifieddetrFilter detection outputs to a specific set of allowed COCO category IDs.

Filter detection outputs to a specific set of allowed COCO category IDs.

Inputs

| Port           | Dtype     | Shape        | Description                    |
| -------------- | --------- | ------------ | ------------------------------ |
| `bboxes`       | `float32` | `[1, -1, 4]` | Detection boxes [1, N, 4] xyxy |
| `category_ids` | `int64`   | `[1, -1]`    | Detection class IDs [1, N]     |
| `confidences`  | `float32` | `[1, -1]`    | Detection scores [1, N]        |

Outputs

| Port           | Dtype     | Shape        | Description                             |
| -------------- | --------- | ------------ | --------------------------------------- |
| `bboxes`       | `float32` | `[1, -1, 4]` | Filtered detection boxes [1, M, 4] xyxy |
| `category_ids` | `int64`   | `[1, -1]`    | Filtered class IDs [1, M]               |
| `confidences`  | `float32` | `[1, -1]`    | Filtered scores [1, M]                  |
...
`DinomalyDetector`cuvis_ai_dinomaly.node.dinomaly_detectorunspecifieddinomaly[↗](https://github.com/cubert-hyperspectral/cuvis-ai-dinomaly "Open plugin repo")Pixel-level anomaly detection using Anomalib's `DinomalyModel`.
...
| Port        | Dtype     | Shape             | Description                                      |
| ----------- | --------- | ----------------- | ------------------------------------------------ |
| `rgb_image` | `float32` | `[-1, -1, -1, 3]` | RGB image [B, H, W, 3] in float32 (0–1 or 0–255) |
...
`QuantizedSpectralSearch`cuvis_ai_turbovec.nodeunspecifiedturbovec

`RTDETRDetection`cuvis_ai_detr.nodeunspecifieddetrRun RT-DETR forward pass and expose raw decoder tensors.

Run RT-DETR forward pass and expose raw decoder tensors.
...
| Port         | Dtype     | Shape          | Description                                                   |
| ------------ | --------- | -------------- | ------------------------------------------------------------- |
| `raw_logits` | `float32` | `[-1, -1, -1]` | Decoder class logits [1, num_queries, num_classes]            |
| `raw_boxes`  | `float32` | `[-1, -1, 4]`  | Decoder box predictions [1, num_queries, 4] cxcywh normalised |
| `orig_hw`    | `int64`   | `[1, 2]`       | Original frame [H, W] before preprocessing                    |

`RTDETRPostprocess`cuvis_ai_detr.nodeunspecifieddetrApply HF RT-DETR postprocessor: sigmoid scoring, box scaling, top-K threshold.

Apply HF RT-DETR postprocessor: sigmoid scoring, box scaling, top-K threshold.

Inputs

| Port         | Dtype     | Shape          | Description                                                             |
| ------------ | --------- | -------------- | ----------------------------------------------------------------------- |
| `raw_logits` | `float32` | `[-1, -1, -1]` | Raw logits from RTDETRDetection [1, num_queries, num_classes]           |
| `raw_boxes`  | `float32` | `[-1, -1, 4]`  | Raw boxes (cxcywh, normalised) from RTDETRDetection [1, num_queries, 4] |
| `orig_hw`    | `int64`   | `[1, 2]`       | Original frame [H, W]                                                   |

Outputs

| Port           | Dtype     | Shape        | Description                                              |
| -------------- | --------- | ------------ | -------------------------------------------------------- |
| `bboxes`       | `float32` | `[1, -1, 4]` | Final boxes [1, N, 4] xyxy in original image coordinates |
| `category_ids` | `int64`   | `[1, -1]`    | Class IDs [1, N]                                         |
| `confidences`  | `float32` | `[1, -1]`    | Confidence scores [1, N]                                 |

`TurboQuantAnomaly`cuvis_ai_turbovec.nodeunspecifiedturbovec
