...
class TestCanvasIntrospection:
    """Test Canvas introspection methods with O(N+E) complexity."""
    ...
    def test_get_input_specs_returns_entry_ports(self):
        ...
        canvas.connect(data_node.outputs.cube, normalizer.inputs.data)

        input_specs = canvas.get_input_specs()
    ...
    def test_get_output_specs_returns_exit_ports(self):
        ...
        output_specs = canvas.get_output_specs()
    ...
    def test_empty_graph_returns_empty_specs(self):
        """Test that empty canvas returns empty specs."""
        canvas = CuvisCanvas("test")
    ...
    def test_single_node_all_ports_unconnected(self):
        ...
        data_node = LentilsAnomalyDataNode(normal_class_ids=[0], name="data")
        canvas._graph.add_node(data_node)
    ...
    def test_complexity_with_large_graph(self):
        ...
        for i in range(len(nodes) - 1):
            if i == 0:
                canvas.connect(nodes[i].outputs.cube, nodes[i + 1].inputs.data)
            else:
                canvas.connect(nodes[i].outputs.normalized, nodes[i + 1].inputs.data)
