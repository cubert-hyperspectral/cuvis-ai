...
def _build_canvas() -> CuvisCanvas:
    canvas = CuvisCanvas("phase3")
    ...
    canvas.connect(source.outputs.data, sink.inputs.data)
    return canvas, source, sink


def test_graphviz_supports_phase3_styling():
    canvas, source, sink = _build_canvas()
...
def test_canvas_visualize_method_proxy():
    canvas, *_ = _build_canvas()

    dot = canvas.visualize(format="graphviz")
    mermaid = canvas.visualize(format="mermaid")

    assert dot.startswith("digraph")
    assert mermaid.startswith("flowchart")


def test_canvas_visualize_can_render_to_file(tmp_path):
    canvas, *_ = _build_canvas()
    ...
    path_md = canvas.visualize(format="render_mermaid", output_path=output_md, wrap_markdown=False)
