...
The embedded pipeline must declare a `plugins:` block (the standalone `configs/pipeline/` yamls already do); a trainrun whose resolved pipeline omits it is rejected at `LoadPipeline` with a `suggest-plugins-fix` hint. For a single-call alternative that does the same server-side, see `RestoreTrainRun` (e.g. `examples/grpc/core/restore_trainrun_grpc.py`).
...
Reads timestamps directly from the Cuvis session held by a `SingleCu3sDataset` and returns `(n_frames - 1) / span_seconds`. This is more reliable than `session.fps` (a nominal value) for spectral cameras where per-frame exposure dominates the real capture cadence.
...
- [Client Connections & Sessions](https://cubert-hyperspectral.github.io/cuvis-ai/0.8.0/deployment/client-connections/index.md)
- [Client Workflows & Error Handling](https://cubert-hyperspectral.github.io/cuvis-ai/0.8.0/deployment/client-workflows/index.md)
- [Profiling & Performance](https://cubert-hyperspectral.github.io/cuvis-ai/0.8.0/workflows/profiling/index.md)
