...
*Multiple connections to one port.* Mark an **input** port `variadic=True`; the node then receives a `list` of values, one per inbound connection. Variadic applies to input ports only — outputs are always single.
