...
2. a **rule** that assigns them to stages (train / val / test / predict), and
3. the **baked** assignment you commit and ship.
...
- **State (universe).** The set of samples that exist and where each physically lives. A module
  produces it from `enumerate()`: either by scanning disk (a `cu3s_multi` glob, `tiff_paired`
...
- **Baked.** A `splits.json` that holds only concrete `file_indices` / `files` plus a pinned
  ...
  [The bake is by generation](#the-bake-is-by-generation)), not by freezing a hand-authored rule.
...
[Two ways a split is produced](#two-ways-a-split-is-produced)).
...
- **Provenance columns are welcome.** Beyond `source, index, path`, a `universe.csv` (or a split CSV
  ...
  reader, and `group` is reserved for group-aware leakage. Keeping such a CSV next to a baked
  `splits.json` is a valid way to retain provenance the baked selectors would otherwise drop.
...
- **`enumerate(required_attrs)`** returns the attributed sample universe. It is two-level: the cheap
...
!!! note "There is no author-then-lower step"
...
- **Module-owned** is for datasets whose split is intrinsically a rule at the group or dataset level
  ...
  when you want a committable, editable artifact.

---

## Leakage, drift, and predict

- **Leakage.** `DataSplitConfig.leakage_check` (`error` / `warn` / `off`, default `error`) asserts
  that train, val, and test are pairwise disjoint by sample `uid`. `predict` is deliberately
...
- [Two-Phase Training](training.md): where a resolved split feeds the trainers.
- [TrainRun Configuration Schema](../reference/configuration/trainrun-schema.md): the `data` block,
  `data.splits`, and `data.params.universe_csv`.
