...
The Reed–Xiaoli (RX) detector is the classical baseline for unsupervised
hyperspectral anomaly detection. It scores each pixel by its
Mahalanobis distance from the background spectrum, so pixels that are
spectrally unusual stand out.

This tutorial walks through building an end-to-end RX pipeline in
cuvis.AI — loading cu3s data, fitting the detector with
[statistical training](../../concepts/training.md), and rendering an
anomaly heatmap.
...
- How [statistical nodes](../../concepts/training.md) accumulate background statistics during initialization.
