...
Hyperspectral cameras often deliver 60–100 bands, but only a handful
carry the signal you actually care about. The Channel Selector node
learns which bands to keep using a two-phase training schedule:
statistical warm-up followed by gradient-based refinement.

This tutorial walks through configuring and training a channel
selector, then inspecting which wavelengths survived.
...
- How [two-phase training](../../concepts/training.md) combines statistical and gradient stages.
