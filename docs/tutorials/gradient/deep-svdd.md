...
Deep Support Vector Data Description (Deep SVDD) learns a compact
representation of "normal" data, then flags anything that lands far
from the centre at inference time. It's the gradient-trained sibling
of [RX anomaly detection](../statistical/rx-anomaly.md) — slower to
train, but able to capture non-Gaussian backgrounds.

This tutorial walks through training Deep SVDD end-to-end on a
hyperspectral dataset and rendering anomaly heatmaps for each frame.
...
- How the [gradient training stage](../../concepts/training.md) consumes a statistically-initialised pipeline.
