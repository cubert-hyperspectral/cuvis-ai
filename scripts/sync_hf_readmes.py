...
The script fetches ``https://huggingface.co/datasets/<org>/<name>/raw/main/README.md``
...
admonition, and writes the result to ``docs/catalogs/datasets/<name>.md``.

Adding a new dataset
--------------------

1. Publish the dataset on HuggingFace under the ``cubert-gmbh`` org.
2. Append its name to the ``DATASETS`` tuple in this module.
...
def _absolutise_assets(text: str, dataset: str) -> str:
    base = RESOLVE_URL_BASE.format(org=ORG, name=dataset)
...
def _header(dataset: str) -> str:
    page = HF_PAGE_URL.format(org=ORG, name=dataset)
    return (
        f'!!! info "Mirrored from HuggingFace"\n'
        f"    This page mirrors the README of [`{ORG}/{dataset}`]({page}).\n\n"
    )


def _empty_body_fallback(dataset: str) -> str:
    page = HF_PAGE_URL.format(org=ORG, name=dataset)
    return (
        f"# {dataset}\n\n"
        f"The upstream dataset card on HuggingFace is currently sparse — "
        f"the dataset is available at [`{ORG}/{dataset}`]({page}) but the "
        f"README has no descriptive body yet. Refer to the linked HuggingFace "
        f"page for files, splits, and download instructions.\n"
    )


def sync_one(dataset: str) -> Path:
    url = RAW_URL.format(org=ORG, name=dataset)
    print(f"  fetching {url}")
    body = _fetch(url)
    body = _strip_frontmatter(body)
    body = _absolutise_assets(body, dataset)
    if not body.strip():
        body = _empty_body_fallback(dataset)
    output = OUTPUT_DIR / f"{dataset}.md"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(_header(dataset) + body, encoding="utf-8")
