...
_FONT_5x7: dict[str, list[str]] = {
    ...
    # Uppercase A-Z, classic 5x7 bitmap font (letters fold lowercase -> uppercase in _glyph).
...
def _glyph(text: str, device: torch.device) -> torch.Tensor:
    ...
    for idx, char in enumerate(text):
        # The bitmap font is uppercase-only; fold lowercase to uppercase before the space fallback.
        rows = _FONT_5x7.get(char) or _FONT_5x7.get(char.upper(), _FONT_5x7[" "])
        glyph = torch.tensor(
