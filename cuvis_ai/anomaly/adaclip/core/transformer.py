"""Transformer components for AdaCLIP.
...
class LayerNormFp32(nn.LayerNorm):
    """LayerNorm that handles fp16 by casting to float32 and back."""
...
class LayerNorm(nn.LayerNorm):
    """LayerNorm with cast back to input dtype."""
...
class QuickGELU(nn.Module):
    """Quick GELU activation (slower than nn.GELU but matches OpenAI CLIP)."""
...
class LayerScale(nn.Module):
    """Layer scale module for transformer blocks."""
...
class ResidualAttentionBlock(nn.Module):
    """Residual attention block for transformers."""
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list) -> tuple[torch.Tensor, list]:
...
class TextTransformer(nn.Module):
    ...
    def _repeat(self, t: torch.Tensor, N: int) -> torch.Tensor:
    ...
    def forward(self, text: torch.Tensor) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, _ = self.transformer(x, attn_mask=attn_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD
