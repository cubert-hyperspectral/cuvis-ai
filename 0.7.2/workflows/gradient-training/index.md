...
Produce a saved, fully-trained pipeline (and a matching `trainrun.yaml`) that can be replayed with [`restore-trainrun`](https://docs.cuvis.ai/0.7.2/workflows/restore-pipeline/index.md) for reproducible re-runs.

## Prerequisites

- A pipeline with at least one node carrying trainable parameters (Deep SVDD, AdaCLIP, learned Channel Selector, …).
- A pipeline that has already been statistically initialised — see [Statistical Training](https://docs.cuvis.ai/0.7.2/workflows/statistical-training/index.md). Gradient training is Phase 2 of the two-phase model.
...
- **Sweep configurations**: pair with [Hydra sweeps](https://docs.cuvis.ai/0.7.2/reference/configuration/hydra-sweeps/index.md) to run a grid of trainings.

## Related

- [Concepts → Training](https://docs.cuvis.ai/0.7.2/concepts/training/index.md) — two-phase model behind the trainer.
- [Concepts → Execution stages](https://docs.cuvis.ai/0.7.2/concepts/execution-stages/index.md) — which nodes run when.
- [Monitoring & Visualization](https://docs.cuvis.ai/0.7.2/workflows/monitoring/index.md) — TensorBoard, callbacks, runtime visualisation.
- [Profiling](https://docs.cuvis.ai/0.7.2/workflows/profiling/index.md) — find bottlenecks in long training runs.
